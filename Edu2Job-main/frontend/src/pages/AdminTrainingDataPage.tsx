...
const AdminTrainingDataPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get("/training-data/");
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsg({ type: "success", text: res.data?.message || "CSV Uploaded & Data Added Successfully!" });
      setFile(null);
      // Reset file input
      ...
      setMsg({ type: "error", text: err.response?.data?.error || err.response?.data?.message || "Upload failed" });
    } finally {
      setUploading(false);
    }
  };
