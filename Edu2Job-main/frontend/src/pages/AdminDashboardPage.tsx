...
import { 
  ...
  Loader2, ArrowRight, TrendingUp, Briefcase
} from "lucide-react";
    ...
    <DashboardLayout>
      ...
      <div className="stats-row" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))' }}>
        ...
        <div className="stat-box">
          ...
          <div>
            <h3 style={{margin:0, fontSize:'1.8rem', fontWeight: 700}}>{stats?.total_job_applications || 0}</h3>
            <span style={{color:'#6b7280', fontSize:'0.9rem'}}>Job Applications</span>
          </div>
        </div>

        {/* Training Data Card */}
        <div className="stat-box">
          <div className="stat-icon" style={{background: '#ddd6fe', color: '#7c3aed'}}>
            <Database size={24} />
          </div>
          <div>
      ...
      <div className="content-split">
        ...
        {/* 4. Recent Job Applications Table */}
        <div className="card">
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: '1rem'}}>
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: '0.8rem'}}>
             {stats?.recent_applications?.length > 0 ? (
                 ...
                 <div key={app.id} style={{
                   ...
                   <div style={{flex: 1}}>
                     <div style={{display:'flex', gap: '10px', alignItems: 'center', marginBottom: '4px'}}>
                       <span style={{fontWeight: 600, fontSize: '0.9rem', color: '#1f2937'}}>{app.username}</span>
                       <span style={{fontSize:'0.75rem', background: getStatusColor(app.status).bg, color: getStatusColor(app.status).text, padding: '2px 8px', borderRadius: '4px', fontWeight: 600}}>
                         {app.status}
                       </span>
                     </div>
                     <span style={{color:'#6b7280', fontSize:'0.85rem', display: 'block'}}>{app.job_title} at {app.company}</span>
                   </div>
                   <span style={{fontSize:'0.75rem', background: '#f3f4f6', padding: '2px 8px', borderRadius: '4px', color: '#6b7280'}}>
                     {app.date}
                   </span>
               ...
               <p style={{color: '#9ca3af', fontSize: '0.9rem', textAlign: 'center', padding: '20px'}}>No recent applications found.</p>
             )}
          </div>
        </div>
